...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    if (AppOptions.get("enableUpdatedAddImage")) {
      const newAltConfig = appConfig.newAltTextDialog;
      altTextManager = newAltConfig && newAltConfig.dialog ? new NewAltTextManager(newAltConfig, overlayManager, eventBus) : null;
    } else {
      const altConfig = appConfig.altTextDialog;
      // Check that all required elements exist before creating AltTextManager
      if (altConfig && altConfig.dialog && altConfig.optionDescription && altConfig.optionDecorative && altConfig.textarea && altConfig.cancelButton && altConfig.saveButton) {
        altTextManager = new AltTextManager(altConfig, container, overlayManager, eventBus);
      } else {
        altTextManager = null;
      }
    }
...
}
{
  ...
  var validateFileURL = function (file) {
    // Disabled origin validation - allow any PDF URL for custom viewer
    return;
    if (!file) {
      return;
    }
